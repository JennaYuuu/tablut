...
class BoardWidget extends Pad {
    ...
    static final Color
        ...
        KING_COLOR = new Color(250, 250, 100),
    ...
    private void drawPiece(Graphics2D g, Square s) {
        /** FIXME. */
        if (_board.get(s) == Piece.BLACK) {
            g.setColor(Color.black);
            g.fillOval(cx(s), cy(s), PIECESIZE, PIECESIZE);
            g.setColor(Color.red);
            g.drawOval(cx(s), cy(s), PIECESIZE, PIECESIZE);
        }
        if (_board.get(s) == Piece.WHITE) {
    ...
    private void click(Square s) {
        /** FIXME. */
        _pastSquares.add(s);
        if (_pastSquares.size() == 2) {
            Square fromSq = _pastSquares.get(0);
            Square toSq = _pastSquares.get(1);
            _commands.add(Move.mv(fromSq, toSq).toString());
            _pastSquares.clear();
        }
        repaint();
    }
    ...
    /** the squared has been saved.*/
    private ArrayList<Square> _pastSquares = new ArrayList<>();
